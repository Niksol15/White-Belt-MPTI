...
class Person {
public:
    Person(const string& first_name, const string& last_name, int year): birthday(year){
...
private:
    map<int, string>::const_iterator getActualFirstNameRecord(int year) const{
        if(firstName.empty() || (firstName.begin() -> first) > year){
            return firstName.cend();
        }
        else
            return --(firstName.upper_bound(year));
    }
    map<int, string>::const_iterator getActualLastNameRecord(int year) const{
        if(lastName.empty() || (lastName.begin() -> first) > year){
            return lastName.cend();
        }
        else
            return --(lastName.upper_bound(year));
    }
    string getFirstNameHistory(int year) const{
    ...
    string getLastNameHistory(int year) const{
